#!/usr/bin/env python
"""P3a：抽 Alpha158 特征到周频 as_of 网格（qlib env）。

Alpha158 raw（infer_processors=[]）逐 instrument-batch fetch → 只留 as_of 行 → 映射 qsym→ticker。
输出 data/bakeoff/alpha158_asof.parquet（MultiIndex(as_of,ticker) × 158）。
"""
...
qsym2ts = json.loads((REPO / "data/bakeoff/ticker_qsym_map.json").read_text())  # qsym->ts_code
asof = sorted({d for (d, t) in pd.read_parquet(REPO / "data/panel/alpha_panel_weekly_v5.parquet",
                                               columns=["forward_return_12d"]).index})
asof_set = {pd.Timestamp(a).normalize() for a in asof}
START, END = str(min(asof).date()), str(max(asof).date())
insts = sorted(qsym2ts.keys())
print(f"[p3a] {len(insts)} instruments, {len(asof)} as_of, {START}->{END}")
...
for i in range(0, len(insts), B):
    ...
    parts.append(f)
    print(f"  batch {i//B}: {len(batch)} inst -> {len(f)} as_of rows ({time.time()-t0:.0f}s)")
...
df.columns = [f"a158_{c}" for c in df.columns]   # 前缀防与 base/survivors 碰撞
out = REPO / "data/bakeoff/alpha158_asof.parquet"
df.to_parquet(out)
print(f"[p3a] saved {out} shape={df.shape} ({time.time()-t0:.0f}s)")
print("[p3a] nan frac:", round(float(df.isna().mean().mean()), 3))
